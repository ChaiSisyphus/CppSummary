...
```
### 指针数组与数组指针
[1]int *f()表示这个函数的功能是返回一个地址。  
[2]int(*f)()表示这是一个函数的指针。它要指向一个函数才能有用，指向一个函数之后可以用它来代替该函数。之后使用这个指针相当于使用该函数  
[3]int *a[]是一个指针数组，表示数组中的每一个元素都是一个指针,可以指向一个int类型的数值。  
[4]int (*a)[]这是一个指向多维数组的指针变量。  
