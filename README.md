...
```
### 函数 变量 左值 右值
[1] int *f(){return 0};     int *a[] = {arr1, arr2};
以上两种就是返回地址的函数和数组的声明, 其中变量f/a先与()/[]集合, 形成函数/数组, 然后被定义指针:指针解引用之后为int类型, 
注意: 这种形式只是出现在定义和初始化同时存在的情况下, 因此函数和数组都可以直接看做, 定义+初始化的组合;
如果函数与变量进行类比, 那么函数和变量则是具有相同的逻辑: 函数/变量都有返回值, **因此他们都是右值,需要有一个左值进行接收**  
[2] int (*func)() = f;   int (*arr)[] = a;
func/arr 先于*结合, *为解引用, 组合后称为一个变量, 然后以该变量为基础定义数组/函数, 因此*func/*arr指向一个函数/数组
func/arr为函数/数组指针, 指向一个函数/数组变量, 因此这是一个左值, 需要一个右值进行赋值;
### 指针数组与数组指针
...
### 静态全局变量与全局变量
[1] 全局变量与静态全局变量的存储方式相同, 都存在静态区;  
[2] 静态全局变量的作用域只限制在当前文件, 其他文件中的静态变量不会相会干扰;
